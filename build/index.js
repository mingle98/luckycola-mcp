...
// 注册文件操作工具
server.tool("fileOperation", "对FILE_PATH目录下的文件进行删除、重命名、读取和写入操作，以及图片压缩", {
        ...
        switch (operation) {
                ...
                try {
                    ...
                    if (onlyFiles)
                        files = files.filter(f => {
                            try {
                                return fs.statSync(path.resolve(FILE_PATH, f)).isFile() && !f.startsWith('.');
                            }
                            catch {
                                return false;
                            }
                ...
                if (ext === '.gif') {
                    ...
                    try {
                        ...
                        await new Promise((resolve, reject) => {
                            execFile(gifsicle, ['-O3', `--colors`, String(colors), '-o', outFile, filePath], (err) => {
                                if (err)
                                    reject(err);
                                else
                                    resolve(true);
            ...
            case "ocrToImageBase64": {
                ...
                try {
                    ...
                    let imageOCRRes = await imageOCRFn(dataUrl, LUCKYCOLA_OPEN_KEY, LUCKYCOLA_OPEN_UID) || {};
